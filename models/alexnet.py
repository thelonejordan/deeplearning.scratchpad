...
class AlexNetModel(nn.Module):
  def __init__(self, num_classes: int=1000, dropout: float=0.5):
    ...
    self.features = nn.Sequential(
      ...
      nn.ReLU(inplace=True),
      nn.Conv2d(384, 256, kernel_size=3, padding=1),  # NOTE: the out channels here are 256 instead of 384 as in the paper
      nn.ReLU(inplace=True),
...
def build(num_classes: int=1000):
  config = AlexNetConfig(num_classes=num_classes)
...
class AlexNet:
  ...
  def from_pretrained(num_classes: int=1000) -> AlexNet:
    import torchvision.models as tvm
    weights = tvm.get_weight(f"AlexNet_Weights.DEFAULT")
    state_dict = weights.get_state_dict()
    model, config = build(num_classes=num_classes)
