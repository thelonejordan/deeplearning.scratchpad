...
def precompute_freqs_cis(dim: int, end: int, theta: float = 10000.0) -> Tensor:
  # 1/(theta ^ 2d) for each d < dim/2
  freqs = 1.0 / (theta ** (torch.arange(0, dim, 2)[: (dim // 2)].float() / dim))
  t = torch.arange(end, device=freqs.device)
  # m/(theta ^ 2d) for each m < end, d < dim/2
...
def apply_rotary_emb_alt(xq: Tensor, xk: Tensor, freqs_cis: Tensor) -> tuple[Tensor, Tensor]:
  ...
  freqs_cis = reshape_for_broadcast(freqs_cis, xq_)  # (seqlen, 1, head_dim/2), same as freqs_cis[:, None, :]
