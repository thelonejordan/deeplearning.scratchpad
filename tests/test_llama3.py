...
def huggingface_run(prompts: list[str], model_desc: str="3B", version: str="2"):
  ...
  model = LlamaForCausalLM.from_pretrained(repo_id, dtype="float16").to(DEVICE)
...
def self_run(prompts: list[str], model_desc: str="3B", version: str="2"):
  ...
  model, tokenizer, max_seq_len, max_batch_size, pad_id, stop_tokens = generator.args
  pad_id = tokenizer.eos_id
  inputs = [tokenizer.encode(s, bos=True, eos=False) for s in prompts]
  outputs, _ = generate(
    model, max_seq_len, max_batch_size, pad_id, stop_tokens, inputs, max_gen_len=max_seq_len, temperature=0.0, echo=True)
